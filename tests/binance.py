...
from cryptobot.enums import JobType, PositionType
from cryptobot.model import Position, add_position, get_position
...
PREVIOUS_LONG = Position(time=datetime.now(), symbol="TEST", position=PositionType.LONG)
...
JOBS = [
    MockJob(name="BTCUSDT,LTCUSDT,XRPUSDT", id=1),
    MockJob(name=JobType.PORTFOLIO_MANAGER, id=3),
]
...
class TestData(unittest.TestCase):
    ...
    @mock.patch("binance.client.Client.get_orderbook_ticker", return_value=TICKER)
    @mock.patch("binance.client.Client.get_symbol_info", return_value=SYMBOL_INFO)
    def test_buy_invalid_qty(
        self, mock_get_symbol, mock_get_ticker, mock_get_asset_bal, mock_req_post
    ):
        handle_long("SYM", PositionType.NONE)
        mock_get_symbol.assert_called_once()
    ...
    @mock.patch("binance.client.Client.get_orderbook_ticker", return_value=TICKER)
    def test_sell_invalid_qty(
        self, mock_get_ticker, mock_get_asset_bal, mock_get_symbol, mock_req_post
    ):
        handle_exit_positions("SYM", PositionType.LONG)
        mock_get_ticker.assert_called_once()
    ...
    def test_update_equity(
        self, mock_db_add, mock_db_commit, mock_get_bal, mock_req_post
    ):
        update_equity()

        mock_get_bal.assert_called()
